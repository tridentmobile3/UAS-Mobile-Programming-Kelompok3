...
class CameraAbsenActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnClose.setOnClickListener {
            finish() // Ini perintah untuk menutup halaman dan kembali ke Dashboard
        }

        // 2. Logika Tombol Jepret
        val btnCapture = findViewById<MaterialCardView>(R.id.btnCapture)
        btnCapture.setOnClickListener {
            takePhoto() // Panggil fungsi jepret saat ditekan
        }
        ...
        imageCapture.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                override fun onError(exc: ImageCaptureException) {
                    Toast.makeText(this@CameraAbsenActivity, "Gagal absen: ${exc.message}", Toast.LENGTH_SHORT).show()
                }

                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    Toast.makeText(this@CameraAbsenActivity, "Absen Berhasil Masuk!", Toast.LENGTH_LONG).show()
                    finish()
                }
            }
        )
